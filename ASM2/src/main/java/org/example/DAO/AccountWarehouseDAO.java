...
public class AccountWarehouseDAO extends SuperDAO<AccountWarehouse,String> {
    ...
    public List<AccountWarehouse> findByAccountId(String accountId) {
        List<AccountWarehouse> accountWarehouses = new ArrayList<>();
        ...
        try (Connection conn = jdbcHelper.getConnection();
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                accountWarehouses.add(mapRowToEntity(rs));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
